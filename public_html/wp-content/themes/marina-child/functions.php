    ...
    <script>
        (function () {
            ...
            if (!Array.isArray(styles) || !styles.length) {
                console.info('[Marina Child] Stylesheet debug script loaded, but there was nothing to report.');
                return;
            }

            console.group('[Marina Child] Stylesheet debug report');

            styles.forEach(function (style) {
                var elementId = style.handle + '-css';
                var selector = 'link#' + elementId + ', style#' + elementId + ', link[data-wp-style="' + style.handle + '"]';
                var stylesheetEl = document.querySelector(selector);

                if (stylesheetEl && stylesheetEl.href) {
                    console.info('%s [%s] loaded from: %s', style.label, style.handle, stylesheetEl.href);
                    return;
                }

                if (stylesheetEl && stylesheetEl.tagName === 'STYLE') {
                    console.info('%s [%s] is inlined in the document head.', style.label, style.handle);
                    return;
                }

                if (style.enqueued) {
                    console.warn('%s [%s] was enqueued, but no matching <link> element (expected #%s) was found.', style.label, style.handle, elementId);
                    return;
                }

                console.warn('%s [%s] is not enqueued on this page.', style.label, style.handle);
            });
