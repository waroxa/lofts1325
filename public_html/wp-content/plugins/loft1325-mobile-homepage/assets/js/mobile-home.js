(function () {
    ...
    function enhanceSearchForm() {
        ...
        var dateDisplayArrivalValue = null;
        var dateDisplayDepartureValue = null;
        var dateDisplayHint = null;
        ...
        if (dateWrappers.length) {
            ...
            var dateDisplayColumns = document.createElement('span');
            dateDisplayColumns.className = 'loft1325-mobile-home__search-input-columns';

            var arrivalColumn = document.createElement('span');
            arrivalColumn.className = 'loft1325-mobile-home__search-input-column';

            var arrivalLabelEl = document.createElement('span');
            arrivalLabelEl.className = 'loft1325-mobile-home__search-input-label';
            arrivalLabelEl.textContent = arrivalLabel;
            arrivalColumn.appendChild(arrivalLabelEl);

            dateDisplayArrivalValue = document.createElement('span');
            dateDisplayArrivalValue.className = 'loft1325-mobile-home__search-input-date';
            dateDisplayArrivalValue.setAttribute('data-placeholder', 'true');
            dateDisplayArrivalValue.textContent = '—';
            arrivalColumn.appendChild(dateDisplayArrivalValue);

            var divider = document.createElement('span');
            divider.className = 'loft1325-mobile-home__search-input-divider';
            divider.setAttribute('aria-hidden', 'true');

            var departureColumn = document.createElement('span');
            departureColumn.className = 'loft1325-mobile-home__search-input-column';

            var departureLabelEl = document.createElement('span');
            departureLabelEl.className = 'loft1325-mobile-home__search-input-label';
            departureLabelEl.textContent = departureLabel;
            departureColumn.appendChild(departureLabelEl);

            dateDisplayDepartureValue = document.createElement('span');
            dateDisplayDepartureValue.className = 'loft1325-mobile-home__search-input-date';
            dateDisplayDepartureValue.setAttribute('data-placeholder', 'true');
            dateDisplayDepartureValue.textContent = '—';
            departureColumn.appendChild(dateDisplayDepartureValue);

            dateDisplayColumns.appendChild(arrivalColumn);
            dateDisplayColumns.appendChild(divider);
            dateDisplayColumns.appendChild(departureColumn);

            dateDisplay.appendChild(dateDisplayColumns);

            dateDisplayHint = document.createElement('span');
            dateDisplayHint.className = 'loft1325-mobile-home__search-input-hint';
            dateDisplayHint.textContent = datePlaceholder;
            dateDisplay.appendChild(dateDisplayHint);
        ...
        function updateDateDisplay() {
            if (!dateDisplay || !dateDisplayArrivalValue || !dateDisplayDepartureValue) {
                return;
            }
            ...
            if (fromValue) {
                dateDisplayArrivalValue.textContent = fromValue;
                dateDisplayArrivalValue.setAttribute('data-placeholder', 'false');
            } else {
                dateDisplayArrivalValue.textContent = '—';
                dateDisplayArrivalValue.setAttribute('data-placeholder', 'true');
            }
            ...
            if (toValue) {
                dateDisplayDepartureValue.textContent = toValue;
                dateDisplayDepartureValue.setAttribute('data-placeholder', 'false');
            } else {
                dateDisplayDepartureValue.textContent = '—';
                dateDisplayDepartureValue.setAttribute('data-placeholder', 'true');
            }

            var hasBothValues = Boolean(fromValue && toValue);
            dateDisplay.setAttribute('data-has-value', hasBothValues ? 'true' : 'false');

            if (dateDisplayHint) {
                dateDisplayHint.hidden = hasBothValues;
                if (!hasBothValues) {
                    dateDisplayHint.textContent = datePlaceholder;
                }
            }
            ...
            var ariaParts = [];
            ariaParts.push(arrivalLabel + ': ' + (fromValue || '—'));
            ariaParts.push(departureLabel + ': ' + (toValue || '—'));
            dateDisplay.setAttribute('aria-label', ariaParts.join(', '));
        }
