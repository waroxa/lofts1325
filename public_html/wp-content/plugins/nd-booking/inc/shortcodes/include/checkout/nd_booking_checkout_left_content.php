...
$nd_booking_nights_label = sprintf( _n( '%s night', '%s nights', $nd_booking_total_nights, 'nd-booking' ), number_format_i18n( $nd_booking_total_nights ) );

$nd_booking_guest_total = absint( $nd_booking_booking_form_guests );
$nd_booking_guest_label = sprintf( _n( '%s guest', '%s guests', $nd_booking_guest_total, 'nd-booking' ), number_format_i18n( $nd_booking_guest_total ) );
...
$nd_booking_summary_details = array(
    __( 'Check-in', 'nd-booking' )  => $nd_booking_checkin_formatted,
    __( 'Check-out', 'nd-booking' ) => $nd_booking_checkout_formatted,
    __( 'Nights', 'nd-booking' )    => $nd_booking_nights_label,
    __( 'Guests', 'nd-booking' )    => $nd_booking_guest_label,
);
...
if ( $nd_booking_booking_original_price != $nd_booking_booking_form_final_price ) {
    $nd_booking_shortcode_left_content .= '<p class="summary-discount">' . sprintf(
        esc_html__( 'Original price %1$s %2$s • Coupon %3$s', 'nd-booking' ),
...
if ( $nd_booking_average_rate > 0 ) {
    $nd_booking_shortcode_left_content .= '<p class="summary-meta">' . sprintf(
        esc_html__( 'Approx. %1$s %2$s per night', 'nd-booking' ),
