...
function wp_loft_booking_branches_page() {
    ...
    if (isset($_POST['submit_branch'])) {
        ...
        $nd_branch_id_raw   = isset($_POST['nd_booking_branch_id']) ? sanitize_text_field($_POST['nd_booking_branch_id']) : '';

        $existing_settings = [];
        if ($branch_id) {
            $existing_settings = $wpdb->get_var($wpdb->prepare("SELECT settings FROM {$wpdb->prefix}loft_branches WHERE id = %d", $branch_id));
        }
        ...
        $nd_branch_id = '' !== $nd_branch_id_raw ? intval($nd_branch_id_raw) : '';
        if ($nd_branch_id !== '') {
            $settings['nd_booking_branch_id'] = $nd_branch_id;
        } else {
            unset($settings['nd_booking_branch_id']);
        }
        ...
        if ($branch_id) {
            ...
            $updated = $wpdb->update($wpdb->prefix . 'loft_branches', $data, array('id' => $branch_id), array('%s', '%s', '%s', '%s', '%s'), array('%d'));
            if ($updated !== false) {
            ...
            // Insert new branch
            $inserted = $wpdb->insert($wpdb->prefix . 'loft_branches', $data, array('%s', '%s', '%s', '%s', '%s'));
            if ($inserted !== false) {
    ...
    $editing_settings = [];
    if ($editing_branch && !empty($editing_branch->settings)) {
        $decoded_settings = json_decode($editing_branch->settings, true);
        if (is_array($decoded_settings)) {
            $editing_settings = $decoded_settings;
        }
    }
    $editing_nd_branch_id = $editing_settings['nd_booking_branch_id'] ?? '';
    ...
    echo '<tr><th><label for="nd_booking_branch_id">ND Booking Branch ID</label></th>';
    echo '<td><input type="number" name="nd_booking_branch_id" id="nd_booking_branch_id" value="' . esc_attr($editing_nd_branch_id) . '" min="0" step="1" placeholder="Optional"></td></tr>';
    ...
    if ($branches) {
        ...
        echo '<thead><tr><th>ID</th><th>Name</th><th>Location</th><th>Building ID</th><th>ND Branch ID</th><th>Search Description</th><th>Actions</th></tr></thead><tbody>';
        foreach ($branches as $branch) {
            $branch_settings = [];
            if (!empty($branch->settings)) {
                $decoded_branch_settings = json_decode($branch->settings, true);
                if (is_array($decoded_branch_settings)) {
                    $branch_settings = $decoded_branch_settings;
                }
            }
            $branch_nd_id = $branch_settings['nd_booking_branch_id'] ?? '';
            echo '<tr>';
            ...
            echo '<td>' . esc_html($branch_nd_id) . '</td>';
...
function wplb_branches_page() {
    ...
    if (isset($_POST['submit_branch'])) {
        ...
        if ($branch_id) {
            $updated = $wpdb->update($wpdb->prefix . 'loft_branches', $data, ['id' => $branch_id], ['%s', '%s', '%s', '%s', '%s'], ['%d']);
            ...
            $inserted = $wpdb->insert($wpdb->prefix . 'loft_branches', $data, ['%s', '%s', '%s', '%s', '%s']);
    ...
    echo '<tr><th><label for="nd_booking_branch_id">ND Booking Branch ID</label></th><td><input type="number" name="nd_booking_branch_id" id="nd_booking_branch_id" value="' . esc_attr($editing_nd_branch_id) . '" min="0" step="1" placeholder="Optional"></td></tr>';
    ...
    $branches = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}loft_branches");
    if ($branches) {
        echo '<table class="widefat fixed" cellspacing="0"><thead><tr><th>ID</th><th>Name</th><th>Location</th><th>Building ID</th><th>ND Branch ID</th><th>Search Description</th><th>Actions</th></tr></thead><tbody>';
        foreach ($branches as $branch) {
            $branch_settings = [];
            if (!empty($branch->settings)) {
                $decoded_branch_settings = json_decode($branch->settings, true);
            ...
            $branch_nd_id = $branch_settings['nd_booking_branch_id'] ?? '';
            echo "<tr><td>{$branch->id}</td><td>{$branch->name}</td><td>{$branch->location}</td><td>{$branch->building_id}</td><td>" . esc_html($branch_nd_id) . "</td><td>{$branch->search_description}</td><td>";
